...
if __name__ == "__main__":
    ...
    actor = blocks.MLP_bounds(nx, nu, bias=True,
                    ...
                    min = -10.0, max = 10.0,
                    hsizes=[256, 256, 256]) # [256, 256] & GELU is nice and smooth
    ...
    td_target = Node(lambda x, l: l + args.gamma*qf1(x, actor(x)), ['X_next', 'l'], ['target_V'])

    # td_error = lambda x, y: 0.5*((qf1(x, actor(x))-y.detach())**2).sum(axis=1, keepdims=True)
    td_error = lambda x, u, y: 0.5*((qf1(x,u)-y)**2).sum(axis=1, keepdims=True)

    # td_target = Node(lambda x, l: l + args.gamma*qf1(x, actor(x)), ['X', 'l'], ['target_V'])

    td_cost = Node(td_error, ['X', 'U', 'target_V'], ['critic_cost'])

    # closed loop system definition
    cl_system = System([dynamics, observation, cost, value, td_target, td_cost], nsteps=1)
    cl_system_eval = System([policy, dynamics, observation, cost, value, td_target, td_cost, X_shift], nsteps=100)
    ...
    policy_obj = PenaltyLoss([l_loss], []) # state_lower_bound_penalty, state_upper_bound_penalty
    ...
    init_data = torch.rand((args.num_data, 1, nx))*20.0 - 1.0
    # init_data[:,:,[1,2,4,5]] *= 2.0

    rand_actions = torch.rand((args.num_data, 1, nu))*20.0 - 10.0

    data = DictDataset({'X': init_data, 'U': rand_actions}, name='train') # Split conditions into train and dev

    init_dev_data = torch.rand((10000, 1, nx))*20.0 - 1.0
    # init_dev_data[:,:,[1,2,4,5]] *= 2.0

    rand_actions_dev = torch.rand((10000, 1, nu))*20.0 - 10.0

    dev_data = DictDataset({'X': init_dev_data, 'U': rand_actions_dev}, name='dev') # Split conditions into train and dev
    ...
    for epoch in range(args.total_epochs):
        ...
        print("EPOCH ", epoch)
        # init_data = torch.rand((args.num_data, 1, nx))*20.0 - 10.0
        # init_data[:,:,[1,2,4,5]] *= 2.0
        ...
        with torch.no_grad():
            ...
            # print(traj_data['target_V'].shape)
