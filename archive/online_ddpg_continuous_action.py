...
if __name__ == "__main__":
    ...
    policy_explore = Node(lambda x: actor(x) + 0.1*torch.randn_like(actor(x)), ['X'], ['U'], name='policy')
    ...
    td_error = lambda x, y: 0.5*((qf1(x)-y.detach())**2).sum(axis=1, keepdims=True)

    td_cost = Node(td_error, ['X', 'target_V'], ['critic_cost'])

    # closed loop system definition
    cl_system = System([policy_explore, dynamics, observation, cost, X_shift, td_target, td_cost], nsteps=100)
    cl_system_1step = System([policy, dynamics, observation, cost, X_shift, td_target, td_cost], nsteps=1)

    cl_system_eval = System([policy, dynamics, observation, cost, X_shift, td_target, td_cost], nsteps=100)
    ...
    policy_problem = Problem([cl_system_1step], policy_obj)
    ...
    for epoch in range(args.total_epochs):
        ...
        init_data = torch.zeros((1000, 1, nx))
        init_data[0:500,:,1:3] = torch.pi #* (torch.rand_like(td[0:split,:,1:3])*0.2 + 0.9)
        # data = DictDataset({'X': 0.1*torch.randn((1000, 1, nx)) + init_data}, name='train')

        ep_data = cl_system({'X': 0.1*torch.randn((1000, 1, nx)) + init_data})
        # print(ep_data['X'].detach().reshape(-1, 1, nx).shape)
        data = DictDataset({'X': ep_data['X'].detach().reshape(-1, 1, nx)})
