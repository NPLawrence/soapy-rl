...
if __name__ == "__main__":
    ...
    nx, nu = 2, 1
    actions = 5*torch.linspace(-1.0, 1.0, steps=11)
    ...
    observation = Node(lambda x: torch.cos(x[:,[0]]), ['X'], ['Y'])
    ...
    qf1 = blocks.MLP_bounds(nx, na, bias=True,
                    ...
                    min = -1/(1-args.gamma), max = 1/(1-args.gamma),
    ...
    cl_system_eval = System([policy, control_action, dynamics, observation, cost, td_target, td_cost, X_shift], nsteps=100)
    ...
    init_data = torch.randn((args.num_data, 1, nx))*5.0
    init_actions = torch.randint(0, na, (args.num_data, 1, 1))


    data = DictDataset({'X': init_data, 'A': init_actions}, name='train')
    ...
    dev_data = DictDataset({'X': init_dev_data, 'A': init_dev_actions}, name='dev')

    eval_init = torch.zeros((100, 1, nx))
    eval_init[:,:,0] = torch.pi
    ...
    for epoch in range(args.total_epochs):
        ...
        with torch.no_grad():
            ...
            axes[1].plot(traj_data['U'][0,:,0].detach().numpy())
            axes[2].plot(traj_data['X'][0,:,0].detach().numpy())
            axes[3].plot(traj_data['X'][0,:,1].detach().numpy())
